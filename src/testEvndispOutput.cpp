...
int main( int argc, char* argv[] )
{
    ...
    if( fProduction == "prod5-South" )
    {
        for( unsigned int i = 0; i < telconfig->GetEntries(); i++ )
        {
            ...
            if( TelID >= 0 && TelID <= 3 )
            {
                checkTelType( TelType, 138704810, fRootFile, TelID );
            }
            else if( TelID >= 4 && TelID <= 28 )
            {
                checkTelType( TelType, 10408618, fRootFile, TelID );
            }
            else if( TelID >= 29 && TelID <= 98 )
            {
                checkTelType( TelType, 201409917, fRootFile, TelID );
            }
            else if( TelID >= 99 && TelID <= 123 )
            {
                checkTelType( TelType, 10608418, fRootFile, TelID );
            }
            else if( TelID >= 124 && TelID <= 126 )
            {
                checkTelType( TelType, 10408618, fRootFile, TelID );
            }
            else if( TelID >= 127 && TelID <= 129 )
            {
                checkTelType( TelType, 10608418, fRootFile, TelID );
            }
            else if( TelID >= 130 && TelID <= 170 )
            {
                checkTelType( TelType, 201409917, fRootFile, TelID );
            }
        }
    ...
    else if( fProduction == "prod5-South-BL-MSTN" )
    {
        for( unsigned int i = 0; i < telconfig->GetEntries(); i++ )
        {
            ...
            else if( TelID >= 4 && TelID <= 73 )
            {
                checkTelType( TelType, 201409917, fRootFile, TelID );
            }
            else if( TelID >= 74 && TelID <= 98 )
            {
                checkTelType( TelType, 10608418, fRootFile, TelID );
            }
        }
    ...
    else if( fProduction == "prod5-North" )
    {
        for( unsigned int i = 0; i < telconfig->GetEntries(); i++ )
        {
            ...
            else if( TelID >= 4 && TelID <= 18 )
            {
                checkTelType( TelType, 10608418, fRootFile, TelID );
            }
            else if( TelID >= 19 && TelID <= 33 )
            {
                checkTelType( TelType, 10408618, fRootFile, TelID );
            }
            else if( TelID >= 34 && TelID <= 46 )
            {
                checkTelType( TelType, 10608418, fRootFile, TelID );
            }
            else if( TelID >= 47 && TelID <= 59 )
            {
                checkTelType( TelType, 10408618, fRootFile, TelID );
            }
            else if( TelID >= 60 && TelID <= 62 )
            {
                checkTelType( TelType, 10608418, fRootFile, TelID );
            }
            else if( TelID >= 63 && TelID <= 65 )
            {
                checkTelType( TelType, 10408618, fRootFile, TelID );
            }
            else if( TelID >= 66 && TelID <= 74 )
            {
                checkTelType( TelType, 10608418, fRootFile, TelID );
            }
            else if( TelID >= 75 && TelID <= 83 )
            {
                checkTelType( TelType, 10408618, fRootFile, TelID );
            }
        }
    ...
    else if( fProduction == "prod3b-paranal-SCT156Tel" )
    {
        for( unsigned int i = 0; i < telconfig->GetEntries(); i++ )
        {
            ...
            else if( TelID >= 41 && TelID <= 114 )
            {
                checkTelType( TelType, 201309316, fRootFile, TelID );
            }
            else if( TelID >= 114 && TelID <= 154 )
            {
                checkTelType( TelType, 207308707, fRootFile, TelID );
            }
        }
    }
    ...
    else if( fProduction == "prod6-South" )
    {
        for( unsigned int i = 0; i < telconfig->GetEntries(); i++ )
        {
            ...
            else if( TelID >= 4 && TelID <= 36 )
            {
                checkTelType( TelType, 10408618, fRootFile, TelID );
            }
            else if( TelID >= 37 && TelID <= 86 )
            {
                checkTelType( TelType, 201109916, fRootFile, TelID );
            }
            else
            {
                cout << "Error: unexpected telescope of type " << TelType;
    ...
    else if( fProduction == "prod6-SouthSCT" )
    {
        for( unsigned int i = 0; i < telconfig->GetEntries(); i++ )
        {
            ...
            else if( TelID >= 87 && TelID <= 105 )
            {
                checkTelType( TelType, 205008707, fRootFile, TelID );
            }
            else
            {
                cout << "Error: unexpected telescope of type " << TelType;
