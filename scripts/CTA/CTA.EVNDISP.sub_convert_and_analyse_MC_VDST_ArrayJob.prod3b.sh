...
then
   echo
   echo "./CTA.EVNDISP.sub_convert_and_analyse_MC_VDST_ArrayJob.prod3b.sh <sub array list> <list of sim_telarray files> <particle> <data set> [keep simtel.root files (default off=0)] [log file directory counter] [qsub options]"
   echo
...
ARRAYCUTS="EVNDISP.prod3.reconstruction.runparameter.NN.noLL"
############################################################################
...
# default (prod3b)
PEDFIL="$CTA_USER_DATA_DIR/analysis/AnalysisData/prod3-Calibration/prod3b.Paranal-20171214/prod3b.Paranal-20171214.ped.root"
if [ $DSET = "prod3-LaPalmaq05-TS" ]
then
   PEDFIL="$CTA_USER_DATA_DIR/GRID_tarballs/20170101.prod3bLaPalma/Calibration/gamma_20deg_180deg_run3___cta-prod3-lapalma3-2147m-LaPalma.ped.root"
fi
echo "PEDFIL: $PEDFIL"
...
# smaller step size for on source gammas
    ...
    then
        ...
        then
            ...
            qsub $QSUBOPT -t $RUNFROMTO:1  -l h_cpu=11:29:00 -l os=sl6 -l tmpdir_size=40G -l h_rss=8G -V -o $QLOG -e $QLOG "$FNAM.sh" 
        else
