...
# execute converter
   ...
   SINGULARITYENV_LD_LIBRARY_PATH=$LD_LIBRARY_PATH singularity exec /project/singularity/images/SL6.img $EVNDISPSYS/bin/CTA.convert_hessio_to_VDST $COPT -a $DETGEO -o $TMPDIR/$OFIL.root $SIMFIL >& $TMPDIR/$OFIL.$N.convert.log
...
# execute eventdisplay
  ...
  then
      SINGULARITYENV_LD_LIBRARY_PATH=$LD_LIBRARY_PATH singularity exec /project/singularity/images/SL6.img $EVNDISPSYS/bin/evndisp -sourcefile $TMPDIR/$OFIL.root $OPT -outputdirectory $TMPDIR -calibrationdirectory $CDIR >& $TMPDIR/$OFIL.$N.evndisp.log
  else
...
# get runnumber and azimuth and rename output files; mv them to final destination
  if [ -e $TMPDIR/$OFIL.root ]
  then
      MCAZ=`SINGULARITYENV_LD_LIBRARY_PATH=$LD_LIBRARY_PATH singularity exec /project/singularity/images/SL6.img $EVNDISPSYS/bin/printRunParameter $TMPDIR/$OFIL.root -mcaz`
      RUNN=`SINGULARITYENV_LD_LIBRARY_PATH=$LD_LIBRARY_PATH singularity exec /project/singularity/images/SL6.img $EVNDISPSYS/bin/printRunParameter $TMPDIR/$OFIL.root -runnumber`
