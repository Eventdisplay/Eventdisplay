 ...
 * possibilities
 *
 * input: WPPhys Performance root files
 *
 * use with .L $EVNDISPSYS/lib/libVAnaSum.so
 *
 */
...
class CTASensitivityPlot
{
    public:
        ...
        CTASensitivityPlot();
        void addDataSet( string iTitle, string iName, string iArray,
                         double iEmin, double iEmax,
                         int iObsTime, double iOffset,
                         int iColor,
                         string iDataDir = "" );
                                     ...
                                     int iColor, string iDataDir )
{
    ...
    if( iDataDir.size() == 0 )
    {
        fData.back()->fDataDir = fDataDirectory;
    }
    else
    {
        fData.back()->fDataDir = iDataDir;
    }
 ...
 */
vector< int > getNScalings_Layout( string iArrayLayout, string iSite = "" )
{
    vector< int > iA;
    if( iSite.find( "prod3b" ) != string::npos )
    {
        iA.push_back( -99 );
        return iA;
    }
 ...
 * for Staging arrays: 2-3
 */
vector< int > getNScalings( string iArrayType, string iSite = "" )
{
    vector< int > iA;
    if( iSite.find( "prod3b" ) != string::npos )
    {
...
vector< string > getArrayLayoutLists( string iArrayType, string iTelTypeCombination )
{
    vector< string > iA;
    /////////////////////////////////////////////////////////////
    // Paranal Layouts (prod3b)
    if( iArrayType == "Baseline Layouts (3b)" )
    {
        iA.push_back( "S.3HB9-" + iTelTypeCombination );
        //        iA.push_back( "S.3HB89-" + iTelTypeCombination );
        //        iA.push_back( "S.3HB8-" + iTelTypeCombination );
        iA.push_back( "S.3HB1-2-" + iTelTypeCombination );
        iA.push_back( "S.3HB4-2-" + iTelTypeCombination );
        iA.push_back( "S.3HB2-2-" + iTelTypeCombination );
    }
    else if( iArrayType == "TS Layouts (3b)" )
    {
        iA.push_back( "S.3HB9-TS-AA-" + iTelTypeCombination );
        iA.push_back( "S.3HB9-TS-AB-" + iTelTypeCombination );
        iA.push_back( "S.3HB9-TS-BA-" + iTelTypeCombination );
        iA.push_back( "S.3HB9-TS-BB-" + iTelTypeCombination );
        iA.push_back( "S.3HB9-TS-CA-" + iTelTypeCombination );
        iA.push_back( "S.3HB9-TS-CB-" + iTelTypeCombination );
        iA.push_back( "S.3HB9-" + iTelTypeCombination );
    }
    /////////////////////////////////////////////////////////////
    // Paranal Layouts
    else if( iArrayType == "Baseline Layouts" )
    {
...
string getDataDir( string iSite, string iArrayLayout = "" )
{
    ...
    else if( iSite == "prod3b-paranal20degp05-NN"
             || iSite == "prod3b-paranal20degq05-NN" )
    {
        if( iArrayLayout.find( "HB2" ) != string::npos
                || iArrayLayout.find( "HB4" ) != string::npos )
        {
            return "prod3b.20deg.20161015/DESY.d20160925m3.V5.";
        }
        return "prod3b.20deg.20161015/DESY.d20160925m4.V5.";
    }
    else if( iSite == "prod3b-paranal40degp05-NN"
             || iSite == "prod3b-paranal40degq05-NN" )
    {
        if( iArrayLayout.find( "HB2" ) != string::npos
                || iArrayLayout.find( "HB4" ) != string::npos )
        {
            return "prod3b.40deg.20161015/DESY.d20160925m3.V5.";
        }
        return "prod3b.40deg.20161015/DESY.d20160925m4.V5.";
    }
...
/*
 * return test describing analysis method
 * and multiplicities
 *
 */
string getNIMText( string iSite, string iArrayLayout = "" )
{
    if( iSite == "prod3b-paranal20degp05-NN"
            || iSite == "prod3b-paranal20degq05-NN"
            || iSite == "prod3b-paranal40degp05-NN"
            || iSite == "prod3b-paranal40degq05-NN" )
    {
        if( iArrayLayout.find( "HB2" ) != string::npos
                || iArrayLayout.find( "HB4" ) != string::npos )
        {
            return "NIM2MST3LST3SST3";
        }
        return "NIM2MST4LST4SST4";
    }
...
void print_titlePage( string iPrint, string iText, float iOffaxisAngle = 0. )
{
    ...
    if( iPrint.size() > 0 )
    {
        ...
        t->SetTextColor( 50 );
        t->SetTextFont( 52 );
        t = iPT->AddText( "DESY analysis - 2016-10-17" );
        t->SetTextColor( 50 );
        t->SetTextFont( 52 );
 ...
 *
 */
void plot_arraylayout( string iArrayLayout, int iScaling = -99, int iTelTypeID = 0,
                       int iObsTime = 50 * 3600,
                       string iSite = "prod3b-paranal20degp05-NN", string iPrint = "" )
{
    char hname[200];
    char htitle[200];
    ...
    string iNIMText = getNIMText( iSite );
    ...
    for( unsigned int p = 0; p < iPointing.size(); p++ )
    {
        if( iScaling < -90 )
        {
            sprintf( htitle, "%s %s %s", iArrayLayout.c_str(), getTTypeTitles( iTelTypeID ).c_str(), getPointingString( iPointing[p] ).c_str() );
        }
        else
        {
            sprintf( htitle, "%s-%d %s %s", iArrayLayout.c_str(), iScaling, getTTypeTitles( iTelTypeID ).c_str(), getPointingString( iPointing[p] ).c_str() );
        }
        sprintf( hname, "ID%d%s%s.%s", iTelTypeID, iPointing[p].c_str(), iNIMText.c_str(), iSite.c_str() );
        
        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout, iScaling ),
                       0.01, E_max_global, iObsTime, 0.0, p + 1 );
    }
    ...
    {
        sprintf( hname, "%s Array %s, %s %.1fh", iSiteText.c_str(), iArrayLayout.c_str(),
                 getTTypeTitles( iTelTypeID ).c_str(),
                 float( iObsTime ) / 3600. );
    }
    ...
    for( unsigned int p = 0; p < iPointing.size(); p++ )
    {
        ...
        for( int j = 0; j < getTelTypes().size(); j++ )
        {
            ...
            for( int i = 1; i <= 5; i++ )
            {
                ...
                fHB248_NG.addDataSet( htitle, hname, harray, 0.01, E_max_global, 180000, iOffaxisAngle, 1 );
                
                sprintf( htitle, "S.3HB4-NG-%d %s", i, getTTypeTitles( getTelTypes()[j] ).c_str() );
                sprintf( hname, "ID%dNIM2.%s", getTelTypes()[j], iSite.c_str() );
                sprintf( harray, "S.3HB4-NG-%d", i );
                
                fHB248_NG.addDataSet( htitle, hname, harray, 0.01, E_max_global, 180000, iOffaxisAngle, 2 );
                ...
                fHB248_NG.addDataSet( htitle, hname, harray, 0.01, E_max_global, 180000, iOffaxisAngle, 4 );
                ...
                for( unsigned int i = 0; i < iScaling.size(); i++ )
                {
                    ...
                    if( getTelTypes( isSouth( iSite ) )[j] != 1 )
                    {
                        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout[l], iScaling[i] ),
                                       0.01, E_max_global, 180000, 0.0, i + 1 );
                    }
                    // plot LST only up to E_max_LST
                    else
                    {
                        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout[l], iScaling[i] ),
                                       0.01, E_max_LST, 180000, 0.0, i + 1 );
                    }
                }
                fA.writeSubArrayList( "subArray.temp.list" );
 ...
 *  array list is of long type
*/
void plot_teltypes( string iSite = "prod3b-paranal20degp05-NN",
                    string iArrayList = "",
                    int iObservationTime = 180000,
                    string iPrint = "" )
{
    char hname[200];
    char htitle[200];
    ...
    for( unsigned int l = 0; l < iArrayLayout.size(); l++ )
    {
        vector< int > iScaling = getNScalings_Layout( iArrayLayout[l], iSite );
        // loop over all pointing directions
        for( unsigned int p = 0; p < iPointing.size(); p++ )
        {
            // 5 scalings
            for( unsigned int i = 0; i < iScaling.size(); i++ )
            {
                ...
                for( int j = 0; j < getTelTypes( isSouth( iSite ) ).size(); j++ )
                {
                    if( iSite.find( "prod3b" ) != string::npos )
                    {
                        sprintf( htitle, "%s %s", iArrayLayout[l].c_str(), getTTypeTitles( getTelTypes()[j] ).c_str() );
                    }
                    else
                    {
                        sprintf( htitle, "%s-%d %s", iArrayLayout[l].c_str(), iScaling[i], getTTypeTitles( getTelTypes()[j] ).c_str() );
                    }
                    sprintf( hname, "ID%d%s.%s", getTelTypes()[j], iNIMText.c_str(), iSite.c_str() );
                    
                    if( getTelTypes()[j] != 1 )
                    {
                        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout[l], iScaling[i] ),
                                       0.01, E_max_global, iObservationTime, 0.0, getTelTypes()[j] + 1 );
                    }
                    // plot LST only up to E_max_LST
                    else
                    {
                        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout[l], iScaling[i] ),
                                       0.01, E_max_LST, iObservationTime, 0.0, getTelTypes()[j] + 1 );
                    }
                }
                fA.writeSubArrayList( "subArray.temp.list" );
                ...
                if( iSite.find( "prod3b" ) != string::npos )
                {
                    sprintf( hname, "%s Array %s, Pointing %s", iSiteText.c_str(),
                             iArrayLayout[l].c_str(),
                             getPointingString( iPointing[p] ).c_str() );
                }
                else
                {
                    sprintf( hname, "%s Array %s, Scaling %d, Pointing %s", iSiteText.c_str(),
                             iArrayLayout[l].c_str(),
                             iScaling[i],
                             getPointingString( iPointing[p] ).c_str() );
                }
                ...
                a.setAxisUnits( getSensitivityAxisUnit( iObservationTime, false ),
                                getSensitivityAxisUnit( iObservationTime, true ) );
                a.setPlotRequirements( iSite, true, iObservationTime );
                a.compareDataSets( "subArray.temp.list", iPointing[p], false, 0, iTitle );
        ...
        for( int j = 0; j < getTelTypes( isSouth( iSite ) ).size(); j++ )
        {
            ...
            for( unsigned int i = 0; i < iScaling.size(); i++ )
            {
                ...
                for( unsigned int p = 0; p < iPointing.size(); p++ )
                {
                    ...
                    {
                        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout[l], iScaling[i] ),
                                       0.011, E_max_global, 180000, 0.0, p + 1 );
                    }
                    // plot LST only up to E_max_LST
                    else
                    {
                        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout[l], iScaling[i] ),
                                       0.01, E_max_LST, 180000, 0.0, p + 1 );
                    }
                }
                fA.writeSubArrayList( "subArray.temp.list" );
                  ...
                  string iTelTypeCombination = "", string iPrint = "", float iOffaxisAngle = 0. )
{
    ...
    vector< int > iNScaling = getNScalings( iArrayType, iSite );
            ...
            for( unsigned int i = 0; i < iNScaling.size(); i++ )
            {
                ...
                for( int j = 0; j < getTelTypes( isSouth( iSite ) ).size(); j++ )
                {
                    ...
                    for( unsigned int l = 0; l < iArrayLayout.size(); l++ )
                    {
                        ...
                        if( iSite.find( "prod3b" ) == string::npos
                                && ( iArrayLayout[l].find( "HB8" ) != string::npos || iArrayLayout[l].find( "HF8" ) != string::npos ) )
                        {
                            iScaling = 1;
                        }
                        if( iScaling > 0 )
                        ...
                        {
                            sprintf( htitle, "%s %s %s", iArrayLayout[l].c_str(),
                                     getTTypeTitles( getTelTypes( isSouth( iSite ) )[j] ).c_str(),
                                     getPointingString( iPointing[p] ).c_str() );
                        }
                        iNIMText = getNIMText( iSite, iArrayLayout[l] );
                        sprintf( hname, "ID%d%s.%s", getTelTypes( isSouth( iSite ) )[j], iNIMText.c_str(), iSite.c_str() );
                        
                        fA.addDataSet( htitle, hname, getArrayLayoutString( isSouth( iSite ), iArrayLayout[l], iScaling ),
                                       0.01, E_max_global, iObservationTime, iOffaxisAngle, l + 1, getDataDir( iSite, iArrayLayout[l] ) );
                    }
                    fA.writeSubArrayList( "subArray.temp.list" );
                    ...
                    {
                        sprintf( hname, "%s %s, Pointing %s %s", iSiteText.c_str(), iTelTypeCombinations[t].c_str(),
                                 getPointingString( iPointing[p] ).c_str(),
                                 getTTypeTitles( getTelTypes( isSouth( iSite ) )[j] ).c_str() );
                    }
                    string iTitle = hname;
                ...
                for( int j = 0; j < 2; j++ )
                {
                    ...
                    for( unsigned int k = 0; k < iObsTimes.size(); k++ )
                    {
                        ...
                        for( unsigned int l = 0; l < iArrayLayout.size(); l++ )
                        {
                            ...
                            fA.addDataSet( htitle, hname, harray, 0.01, E_max_global, iObsTimes[k], 0.0, l + 1 );
                        }
                        fA.writeSubArrayList( "subArray.temp.list" );
                        ...
                        a.setEnergyRange_Lin_TeV( 0.01, E_max_LST );
                        a.setResolutionLimits( 0.5, 0.4 );
                        a.setSensitivityRatioLimits( 0.4, 1.4 );
...
void paranal_MSTs( string iSite = "prod3-paranalp05-NN", string iArrayList = "", string iPrint = "" )
{
    ...
    // number of scalings
    vector< int > iNScalings_v = getNScalings( "", iSite );
    unsigned int iNScalings = iNScalings_v.size();
                ...
                for( int i = 1; i <= iNScalings; i++ )
                {
                    ...
                    for( unsigned int s = 0; s < getSSTTypes().size(); s++ )
                    {
                        ...
                        for( unsigned int j = 0; j < getMSTTypes().size(); j++ )
                        {
                            ...
                            if( iSite.find( "prod3b" ) != string::npos )
                            {
                                sprintf( htitle, "%s %s", iArrayLayoutName.c_str(), getTTypeTitles( iRecID[r] ).c_str() );
                                sprintf( harray, "%s", iArrayLayoutName.c_str() );
                            }
                            else
                            {
                            ...
                            sprintf( hname, "ID%d%s.%s", iRecID[r], iNIMText.c_str(), iSite.c_str() );
                            
                            fA.addDataSet( htitle, hname, harray, 0.01, E_max_global, 180000, 0.0, j + 1 );
                        }
                        fA.writeSubArrayList( "subArray.temp.list" );
                        ...
                        if( iSite.find( "prod3b" ) != string::npos )
                        {
                            sprintf( hname, "%s Array %s, SST %s, Poin. %s %s", iSiteText.c_str(), iArrayLayout[l].c_str(),
                                     getSSTTypes()[s].c_str(),
                                     getPointingString( iPointing[p] ).c_str(),
            ...
            for( unsigned int p = 0; p < iPointing.size(); p++ )
            {
                ...
                for( int i = 1; i <= iNScalings; i++ )
                {
                    ...
                    for( unsigned int s = 0; s < getMSTTypes().size(); s++ )
                    {
                        ...
                        if( iSite.find( "prod3b" ) != string::npos )
                        {
                            sprintf( hname, "%s Array %s, MST %s, Poin. %s %s", iSiteText.c_str(), iArrayLayout[l].c_str(),
                                     getMSTTypes()[s].c_str(),
                                     getPointingString( iPointing[p] ).c_str(),
                                     getTTypeTitles( iRecID[r] ).c_str() );
                        }
        ...
        for( unsigned int s = 0; s < iStagingName.size(); s++ )
        {
            ...
            for( unsigned int i = 0; i < iArrayLayout[s].size(); i++ )
            {
                ...
                else
                {
                    if( isSouth( iSite ) )
                    {
                        fA.addDataSet( htitle, hname, harray, 0.01, E_max_global, 180000, 0.0, i + 1 );
                    }
                    else
                    {
 ...
 *
*/
void plot_paranal( int iObservationTime = 180000 )
{
    vector< string > iSite;
    ...
    for( unsigned int i = 0; i < iSite.size(); i++ )
    {
        ...
        for( unsigned int o = 0; o < iOffAxisBins.size(); o++ )
        {
            ...
            for( unsigned int j = 0; j < iTelCombinations.size(); j++ )
            {
                ...
                plot_layout( iSite[i], "Baseline Layouts", iObservationTime, iTelCombinations[j], iPrintName.str(), iOffAxisBins[o] );
                ...
                plot_layout( iSite[i], "Descoped Layouts", iObservationTime, iTelCombinations[j], iPrintName.str(), iOffAxisBins[o] );
            }
            continue;
            // staged layouts
            /*            for( unsigned int j = 1; j < 7; j++ )
                        {
                            ...
                            plot_layout( iSite[i], iStagingName.str(), iObservationTime, "ND", iPrintName.str(), iOffAxisBins[o] );
                        }  */
        }
        continue;
...
void help()
{
    ...
    cout << "void plot_teltypes( string iSite = \"prod3-paranalp05-NN\", string iArrayList = \"\", int iObservationTime = 180000, string iPrint = \"\" )" << endl;
    cout << "\t array list is the typical array list used for the CTA analysis" << endl;
    cout << "---" << endl;
